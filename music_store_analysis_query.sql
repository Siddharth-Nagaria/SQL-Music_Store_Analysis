...
SELECT billing_country, COUNT(*) as numb
FROM invoice
...
-- Q4Which city has the best customers? We would like to throw a promotional Music Festival in the city
-- we made the most money. Write a query that returns one city that has the highest sum of invoice totals.
-- Return both the city name & sum of all invoice totals.

SELECT billing_city, SUM(total) as earning
FROM invoice
...
SELECT c.customer_id, c.first_name, c.last_name, SUM(i.total) as total_payment
FROM customer as c
JOIN invoice as i
...
-- Q1 Write a query to return the email, first name, & Genre of 
...
SELECT AVG(track.milliseconds) as avg_length
FROM track
)
ORDER BY track.milliseconds DESC;

-- Question Set 3 - Advanced 
-- Find how much amount spend by each customer on artists ?
-- Write a query to return customer name, artist name 
-- total spent.

-- In this question we would be creating a CTE that is a temporary table which doesn't exist but we use it for 
